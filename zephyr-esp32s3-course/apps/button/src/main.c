...
static void debounce_work_handler(struct k_work *work)
{
    ...
    /* gpio_pin_get_dt() returns the LOGICAL level, not the raw voltage: it has
     * already applied the GPIO_ACTIVE_LOW inversion from the devicetree.
     * So 1 = active (pressed), 0 = inactive (released). Use the raw accessor
     * gpio_pin_get_raw() if you ever need the actual electrical level instead.
     * (Thanks to Marc Binderberger for catching the inverted test here.)
     */
